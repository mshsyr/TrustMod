...
WifiNetDevice::Send (Ptr<Packet> packet, const Address& dest, uint16_t protocolNumber)
{
  ...
  //std::cout<<"WifiNetDevice node Pointer is: "<<n<<std::endl;
  //std::cout<<"WifiNetDevice trust Pointer is: "<<t<<std::endl;
  if(trust)
  {
  trust->SentInput(packet, realTo);
  //std::cout<<"(wifi-net-device.cc) The Mac address of the sent packet is: "<<realTo<<std::endl;
  }
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
WifiNetDevice::ForwardUp (Ptr<Packet> packet, Mac48Address from, Mac48Address to)
{
  ...
  if (!m_promiscRx.IsNull ())
    {
      //std::cout<<"^^^^^^^^^^^^^^^^^^^WifiNetDevice, "<<" Source: "<<from<<" Destination: "<<to <<" Packet: "<<packet<<std::endl;//////MSH
