...
Trust::ForwardedInput (Ptr<const Packet>  packet, Address from) const
{
	...
	//2. Check if packet source ip is equal to node IP and the packet source mac is not equal to node mac
...
{
	NS_LOG_FUNCTION (this << "Test Output"); 
	Ptr<Node> node = GetNode ();
	Ptr<Trust> trust = node->GetObject<Trust> ();
...
Trust::UpdateForwarded (Mac48Address from)
{
	...
	// There is a map element (vector), but need a new trust node
...
Trust::RecommendationReceive (Ptr<Socket> socket)
{
	...
	while (packet = socket->Recv ())
    {
	  ...
	  std::cout<<std::endl;
	  NS_LOG_UNCOND ("One packet received!");
	  ...
	  else if (packetType == "Res")
	  {
		  ...
		  // 3. Check if this response is for the node
		  ...
		  if (trust->m_trustMap.count(trusteeMac) == 0)  //check if there is already trust record for this request
		  {
